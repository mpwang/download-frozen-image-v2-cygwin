...
handle_single_manifest_v2() {
    ...
    local layerEntry="$(echo '[]' | jq --raw-output ".$(for layerFile in "${layerFiles[@]}"; do echo " + [ \"$layerFile\"  ]"; done)")"
	local jsonEntry=". + {Confiig: \"${configFile}\", RepoTags: [\"${image#library\/}:${tag}\"], Layers: ${layerEntry} }"
	echo "${jsonEntry}" > "$dir/jsonEntry.json"
	local manifestJsonEntry="$(echo '{}'  | jq --raw-output -f ${dir}/jsonEntry.json --)"
...
if [ -z "$doNotGenerateManifestJson" ] && [ "${#manifestJsonEntries[@]}" -gt 0 ]; then
    jsonEntries=".$(for entry in "${manifestJsonEntries[@]}"; do echo " + [ $entry ]"; done)"
	echo ".${jsonEntries}" > "$dir/jsonEntries.json"
	echo '[]' | jq --raw-output -f "$dir/jsonEntries.json" -- > "$dir/manifest.json"
	# echo '[]' | jq --raw-output ".$(for entry in "${manifestJsonEntries[@]}"; do echo " + [ $entry ]"; done)" > "$dir/manifest.json"
else
	rm -f "$dir/manifest.json"
fi
rm -f "$dir/jsonEntry.json" "$dir/jsonEntries.json"
